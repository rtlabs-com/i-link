...
#define _COMPILETIME_ASSERT_SYMBOL_INNER(line_number) __COMPILETIME_ASSERT_ ## line_number
#define _COMPILETIME_ASSERT_SYMBOL(line_number) _COMPILETIME_ASSERT_SYMBOL_INNER(line_number)
#define COMPILETIME_ASSERT(constant_expression) \
   typedef char _COMPILETIME_ASSERT_SYMBOL(__LINE__) [ ((constant_expression) ? 1 : -1) ]
#endif
...
typedef uint8_t iolink_port_qualifier_info_t;  //!< Port Qualifier Information (PQI)
...
typedef enum CC_PACKED iolink_eventcode
{
   ...
   IOLINK_EVENTCODE_DEV_DS_UPLOAD_REQ = 0xFF91, /* From D.1 EventCodes for Devices */
} iolink_eventcode_t;
CC_PACKED_END
...
typedef struct CC_PACKED arg_block_pdinout
{
    ...
    * data[in_len + 2] = out_data0, data[in_len + 3] = out_data1 ... data[in_len + out_len = 1] = out_dataM
    */
...
COMPILETIME_ASSERT (sizeof(arg_block_masterident_t) == sizeof(arg_block_masterident_head_t) + sizeof(iolink_port_types_t) * IOLINK_NUM_PORTS);

COMPILETIME_ASSERT (sizeof(arg_block_pdin_head_t) == 4);
COMPILETIME_ASSERT (sizeof(arg_block_pdin_t) == sizeof(arg_block_pdin_head_t) + IOLINK_PD_MAX_SIZE);

COMPILETIME_ASSERT (sizeof(arg_block_pdout_head_t) == 4);
COMPILETIME_ASSERT (sizeof(arg_block_pdout_t) == sizeof(arg_block_pdout_head_t) + IOLINK_PD_MAX_SIZE);

COMPILETIME_ASSERT (sizeof( arg_block_pdinout_head_t) == 4);
COMPILETIME_ASSERT (sizeof(arg_block_pdinout_t) == sizeof(arg_block_pdinout_head_t) + 2 + 2 * IOLINK_PD_MAX_SIZE);
...
COMPILETIME_ASSERT (sizeof(arg_block_portconfiglist_t) == 2 + sizeof(portconfiglist_t));

COMPILETIME_ASSERT (sizeof(arg_block_portstatuslist_t) == sizeof(diag_entry_t *) + 15);
...
uint8_t iolink_set_config (uint8_t portnumber, uint16_t vid, uint32_t did,
                           uint8_t cycle_time, iolink_portmode_t portmode,
                           iolink_validation_check_t validation,
                           char * serial_number);
uint8_t iolink_reset_events (uint8_t portnumber);
uint8_t iolink_get_events (uint8_t portnumber, uint8_t * diag_count,
...
iolink_error_t SMI_DSToParServ_req (uint8_t portnumber,
                                    iolink_arg_block_id_t exp_arg_block_id,
                                    uint16_t arg_block_len,
                                    arg_block_t * arg_block);
