...
#define VERIFY_ITEM(structure, item, portnumber, text) \
   ...
   {                                                              \
      LOG_WARNING(LOG_STATE_ON,                                   \
                  ...
                  __func__, portnumber, text, (unsigned int)item, \
                  (unsigned int)structure->item);                 \
      return 1;                                                   \
   }

static void SMI_cnf_cb (void * arg, uint8_t portnumber,
                        iolink_arg_block_id_t ref_arg_block_id,
                        uint16_t arg_block_len, arg_block_t * arg_block);

static uint8_t verify_smi_masterident (iolink_app_port_ctx_t * app_port,
                                       uint16_t vendorid, uint32_t masterid)
{
   ...
   iolink_error_t err = SMI_MasterIdentification_req (app_port->portnumber,
                                                      IOLINK_ARG_BLOCK_ID_MASTERIDENT,
                                                      sizeof(arg_block_void_t),
                                                      (arg_block_t *)&arg_block_void);
   if ((err != IOLINK_ERROR_NONE) ||
       (wait_for_cnf(app_port, SMI_MASTERIDENT_CNF, 1000) != IOLINK_SMI_ERRORTYPE_NONE))
   {
      return 1;
   }
...
static void iolink_app_init_port (iolink_app_port_ctx_t * app_port, iolink_m_cfg_t * m_cfg,
                                  const iolink_port_cfg_t * port_cfg)
{
   iolink_port_t * port = iolink_get_port (iolink_app_master.master,
                                           app_port->portnumber);

   app_port->event_mtx = os_mutex_create();
   app_port->status_mtx = os_mutex_create();
   app_port->pdout_mtx = os_mutex_create();
   iolink_dl_instantiate (port, port_cfg, m_cfg->dl_thread_prio, m_cfg->dl_thread_stack_size);
   ...
   {
      LOG_WARNING(LOG_STATE_ON, "%s: Verify MasterIdentification failed\n", __func__);
   }
}

void iolink_common_config (arg_block_portconfiglist_t * port_cfg, uint16_t vid,
                           uint32_t did, uint8_t cycle_time, iolink_portmode_t portmode,
...
static uint8_t iolink_config_port_sdci_auto (iolink_app_port_ctx_t * app_port)
{
   ...
   iolink_common_config(&port_cfg, 0, 0, 0 /* AFAP (As fast as possible) */,
                        IOLINK_PORTMODE_IOL_AUTO, IOLINK_VALIDATION_CHECK_NO);

   app_port->app_port_state = IOL_STATE_STARTING;

   iolink_error_t err = SMI_PortConfiguration_req (app_port->portnumber,
                                                   IOLINK_ARG_BLOCK_ID_VOID_BLOCK,
                                                   sizeof(arg_block_portconfiglist_t),
                                                   (arg_block_t *)&port_cfg);
   if ((err != IOLINK_ERROR_NONE) ||
       (wait_for_cnf(app_port, SMI_PORTCFG_CNF, 1000) != IOLINK_SMI_ERRORTYPE_NONE))
   {
      return 1;
   }
...
static uint8_t iolink_config_port (iolink_app_port_ctx_t * app_port,
...
uint8_t get_port_status (iolink_app_port_ctx_t * app_port)
{
   ...
   iolink_error_t err = SMI_PortStatus_req (app_port->portnumber,
                                            IOLINK_ARG_BLOCK_ID_PORT_STATUS_LIST,
                                            sizeof(arg_block_void_t),
                                            (arg_block_t *)&arg_block_void);
   if ((err != IOLINK_ERROR_NONE) ||
       (wait_for_cnf(app_port, SMI_PORTSTATUS_CNF, 1000) != IOLINK_SMI_ERRORTYPE_NONE))
   {
      return 1;
   }
...
static uint8_t iolink_start_port (iolink_app_port_ctx_t * app_port)
{
   ...
   {
      LOG_WARNING(LOG_STATE_ON, "%s: Port %u is not in state operational, port_status_info = %u\n",
                  __func__, portnumber, port_status->port_status_info);
   ...
   if (port_status->vendorid == IFM_VENDOR_ID)
   {
      switch (port_status->deviceid)
      {
         ...
         default:
            ...
            LOG_WARNING(LOG_STATE_ON, "%s: Port %u: Unknown iolink device 0x%06x for VID 0x%04x\n",
   ...
   {
      app_port->type = UNKNOWN;
      LOG_WARNING(LOG_STATE_ON, "%s: Port %u: Unknown device: Vendor ID = 0x%04X, Device ID = 0x%06X\n",
                  __func__, portnumber, port_status->vendorid, (int)port_status->deviceid);
   }
...
static void PD_cb (uint8_t portnumber, void * arg, uint8_t data_len, const uint8_t * inputdata)
{
...
static void iolink_retry_estcom(os_timer_t *tmr, void * arg)
{
   ...
   iolink_app_master.app_port[port_idx].app_port_state = IOL_STATE_WU_RETRY_WAIT_TSD;
...
void iolink_handler (iolink_m_cfg_t m_cfg)
{
   ...
   for (i = 0; i < IOLINK_NUM_PORTS; i++)
   {
      ...
      {
         iolink_tsd_tmr[i] = os_timer_create(1000 * 1000, iolink_retry_estcom, (void *)i, true);
      }
   }
   ...
   for (i = 0; i < m_cfg.port_cnt; i++)
   {
      if (*m_cfg.port_cfgs[i].mode != iolink_mode_INACTIVE)
      {
         ...
         iolink_app_init_port (&iolink_app_master.app_port[i],
                               &m_cfg, &m_cfg.port_cfgs[i]);
      }
      ...
      if (app_port->allocated == 1)
      {
         ...
         {
            LOG_WARNING(LOG_STATE_ON, "%s: Failed to config port %lu\n", __func__, i + 1);
         }
      }
   }
         ...
         for (i = 0; i < m_cfg.port_cnt; i++)
         {
            ...
            if (((EVENT_PORTE_0 << i) & event_value) != 0)
            {
               if (app_port->app_port_state == IOL_STATE_STARTING)
               {
                  ...
                  {
                     LOG_WARNING(LOG_STATE_ON, "%s: Failed to start port %lu\n", __func__, i + 1);
                  }
               }
               else if (app_port->app_port_state != IOL_STATE_STOPPING)
               {
                  LOG_WARNING(LOG_STATE_ON, "%s: EVENT_PORT for port %lu, when in port_state %u\n",
                              __func__, i + 1, app_port->app_port_state);
               }
            }
            ...
            if (((EVENT_RETRY_ESTCOM_0 << i) & event_value) != 0)
            {
               ...
               {
                  iolink_dl_reset (iolink_get_port (iolink_app_master.master,
                                                    app_port->portnumber));
                  if (iolink_config_port (app_port, port_mode[i]) != 0)
                  {
                     LOG_WARNING(LOG_STATE_ON, "%s: Failed to config port %lu\n", __func__, i + 1);
...
static inline void handle_smi_deviceevent (iolink_app_port_ctx_t * app_port,
                                           arg_block_devevent_t * arg_block_devevent)
{
   if (app_port->type == GOLDEN)
   {
      ...
      for (i = 0; i < arg_block_devevent->event_count; i++)
      {
         ...
         app_port->events.diag_entry[event_index].event_qualifier = entry->event_qualifier;
...
static inline void handle_smi_portevent (iolink_app_port_ctx_t * app_port,
                                         diag_entry_t * event)
{
   uint8_t portnumber = app_port->portnumber;
   ...
   if (((app_port->type == GOLDEN) || (app_port->type == UNKNOWN)) &&
         event->event_code != IOLINK_EVENTCODE_NO_DEV)
   {
      ...
      CC_ASSERT (event_index < PORT_EVENT_COUNT);
      app_port->events.diag_entry[event_index].event_qualifier = event->event_qualifier;
   ...
   if (event->event_code != IOLINK_EVENTCODE_NO_DEV)
   {
      LOG_DEBUG(LOG_STATE_ON, "%s (%d): type = %d, event_code = 0x%04X, count = %d\n",
                __func__, portnumber, app_port->type, event->event_code, app_port->events.count);
   }
   ...
   case IOLINK_EVENTCODE_BAD_VID:
      LOG_WARNING(LOG_STATE_ON, "%s: Port %u: portevent bad vid/did.\n", __func__, portnumber);
      /* Port status changed - Use SMI_PortStatus() for details */
      os_event_set (app_master->app_event, (EVENT_PORTE_0 << port_index));
      break;
   case IOLINK_EVENTCODE_NO_DEV: /* COMLOST */
      if (app_port->app_port_state != IOL_STATE_STARTING)
      {
         LOG_WARNING(LOG_STATE_ON, "%s: Port %u: portevent COMLOST\n", __func__, portnumber);
      }
   ...
   case IOLINK_EVENTCODE_BACKUP_INCON_DL:
      break;
   default:
      LOG_WARNING(LOG_STATE_ON, "%s: Port %u: Unknown SMI_PortEvent code 0x%04x\n",
                  __func__, portnumber, event->event_code);
      break;
   }
}

static inline void handle_smi_joberror (iolink_app_port_ctx_t * app_port,
                                        iolink_arg_block_id_t ref_arg_block_id,
                                        arg_block_joberror_t * arg_block_err)
{
   ...
   {
      LOG_WARNING(LOG_STATE_ON, "%s: Port %u: Read failed, buffer too small.\n",
   ...
   case IOLINK_ARG_BLOCK_ID_DS_DATA:
      ...
      break;
   default:
      LOG_WARNING(LOG_STATE_ON, "%s: Port %u: SMI failed! errortype = 0x%04x, ref = 0x%04x\n",
                  __func__, app_port->portnumber, app_port->errortype, ref_arg_block_id);
      break;
   }
}
                        ...
                        uint16_t arg_block_len, arg_block_t * arg_block)
{
   ...
   case IOLINK_ARG_BLOCK_ID_JOB_ERROR:
      handle_smi_joberror (app_port, ref_arg_block_id,
   ...
   if (!match_found)
   {
      ...
      case IOLINK_ARG_BLOCK_ID_PORT_CFG_LIST:
         ...
         LOG_DEBUG(LOG_STATE_ON, "%s: IOLINK_ARG_BLOCK_ID_PORT_CFG_LIST\n", __func__);
         os_event_set (app_port->event, SMI_PORTCFG_CNF);
         break;
      default:
         LOG_WARNING(LOG_STATE_ON, "%s: Port %u: Unexpected ref_arg_block_id 0x%04x and arg_block_id 0x%04x\n",
                     __func__, portnumber, ref_arg_block_id, arg_block->void_block.arg_block_id);
         break;
      }
   }
}
...
iolink_smi_errortypes_t wait_for_cnf(iolink_app_port_ctx_t * app_port, uint32_t mask, uint32_t ms)
{
   ...
   {
      LOG_WARNING(LOG_STATE_ON, "%s: timeout for port %u (mask = 0x%X)\n", __func__,
                  app_port->portnumber, (unsigned int)mask);
