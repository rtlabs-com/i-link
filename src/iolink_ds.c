...
typedef struct iolink_fsm_ds_transition
{
   ...
   iolink_fsm_ds_event_t (*action)(iolink_port_t * port,
                                   iolink_fsm_ds_event_t event);
...
static void ds_AL_Write_cnf (iolink_port_t * port,
                             iolink_smi_errortypes_t errortype);
static void ds_AL_Read_cnf (iolink_port_t * port, uint8_t len,
                            const uint8_t * data,
                            iolink_smi_errortypes_t errortype);

static inline void ds_AL_Read_req (iolink_port_t * port, uint16_t index,
                                   uint8_t subindex)
{
...
static inline void ds_AL_Write_ds_cmd_req (iolink_port_t * port,
                                           iolink_ds_command_t cmd)
{
   ...
   ds->command = cmd;
   AL_Write_req (port, ds->current_index, ds->current_subindex, 1,
                 (uint8_t *)&ds->command, ds_AL_Write_cnf);
}

static iolink_fsm_ds_event_t ds_delete (iolink_port_t * port,
                                        iolink_fsm_ds_event_t event)
{
   iolink_ds_port_t * ds = iolink_get_ds_ctx (port);
...
static iolink_fsm_ds_event_t ds_todo (iolink_port_t * port,
                                      iolink_fsm_ds_event_t event)
{
   CC_ASSERT (0);
   return DS_EVENT_NONE;
...
static iolink_fsm_ds_event_t ds_ul_fault (iolink_port_t * port,
                                          iolink_fsm_ds_event_t event)
{
   return DS_EVENT_FAULT_UL;
}

static iolink_fsm_ds_event_t ds_dl_fault (iolink_port_t * port,
                                          iolink_fsm_ds_event_t event)
{
   return DS_EVENT_FAULT_DL;
}

static iolink_fsm_ds_event_t ds_fault (iolink_port_t * port,
                                       iolink_fsm_ds_event_t event)
{
   ...
   case DS_EVENT_FAULT_SIZE:
      DS_Fault (port, (event == DS_EVENT_FAULT_ID) ? IOLINK_DS_FAULT_ID : IOLINK_DS_FAULT_SIZE);
      res = DS_EVENT_FAULT_DONE; /* T5 */
      break;
   case DS_EVENT_FAULT_UL:
   case DS_EVENT_FAULT_DL:
      ds->fault = (event == DS_EVENT_FAULT_UL) ? IOLINK_DS_FAULT_UP : IOLINK_DS_FAULT_DOWN;
...
static iolink_fsm_ds_event_t ds_off (iolink_port_t * port,
                                     iolink_fsm_ds_event_t event)
{
...
static bool ds_check_vid_did (iolink_port_t * port)
{
   ...
   if ((ds->master_ds.vid == 0) ||
       ((ds->master_ds.vid == port_info->vendorid) &&
...
static iolink_fsm_ds_event_t ds_chk_id (iolink_port_t * port,
                                        iolink_fsm_ds_event_t event)
{
...
static iolink_fsm_ds_event_t ds_chk_mem_size (iolink_port_t * port,
                                              iolink_fsm_ds_event_t event)
{
   iolink_ds_port_t * ds = iolink_get_ds_ctx (port);
   iolink_fsm_ds_event_t res = DS_EVENT_NONE;
...
static iolink_fsm_ds_event_t ds_chk_upload (iolink_port_t * port,
                                            iolink_fsm_ds_event_t event)
{
   iolink_fsm_ds_event_t res = DS_EVENT_NONE;
   iolink_ds_port_t * ds = iolink_get_ds_ctx (port);
   bool upload_flag = ds->device_ds.state_property & DS_STATE_PROPERTY_UPLOAD_REQ;

   if ((ds->device_ds.state_property & DS_STATE_PROPERTY_STATE_MASK) ==
       DS_STATE_PROPERTY_STATE_LOCKED)
   {
...
static iolink_fsm_ds_event_t ds_upload (iolink_port_t * port,
                                        iolink_fsm_ds_event_t event)
{
   iolink_fsm_ds_event_t res = DS_EVENT_NONE;
   iolink_ds_port_t * ds = iolink_get_ds_ctx (port);
...
static iolink_fsm_ds_event_t ds_chk_validity (iolink_port_t * port,
                                              iolink_fsm_ds_event_t event)
{
   iolink_ds_port_t * ds = iolink_get_ds_ctx (port);
   iolink_fsm_ds_event_t res = DS_EVENT_NONE;
...
static iolink_fsm_ds_event_t ds_chk_csum (iolink_port_t * port,
                                          iolink_fsm_ds_event_t event)
{
   iolink_fsm_ds_event_t res;
   iolink_ds_port_t * ds = iolink_get_ds_ctx (port);
...
static iolink_fsm_ds_event_t ds_write_param (iolink_port_t * port,
                                             iolink_fsm_ds_event_t event)
{
   iolink_ds_port_t * ds = iolink_get_ds_ctx (port);
...
static iolink_fsm_ds_event_t ds_wait_ds_act (iolink_port_t * port,
                                             iolink_fsm_ds_event_t event)
{
   return DS_EVENT_NONE;
}

static iolink_fsm_ds_event_t ds_dl_done (iolink_port_t * port,
                                         iolink_fsm_ds_event_t event)
{
...
static iolink_fsm_ds_event_t ds_readparam (iolink_port_t * port,
                                           iolink_fsm_ds_event_t event)
{
   iolink_ds_port_t * ds = iolink_get_ds_ctx (port);
...
static iolink_fsm_ds_event_t ds_store_data (iolink_port_t * port,
                                            iolink_fsm_ds_event_t event)
{
...
static iolink_fsm_ds_event_t ds_ready (iolink_port_t * port,
                                       iolink_fsm_ds_event_t event)
{
   DS_Ready (port);
...
static iolink_fsm_ds_event_t ds_decompose_set (iolink_port_t * port,
                                               iolink_fsm_ds_event_t event)
{
   iolink_ds_port_t * ds = iolink_get_ds_ctx (port);
   iolink_fsm_ds_event_t res = DS_EVENT_NONE;
...
static iolink_fsm_ds_event_t ds_init_dl (iolink_port_t * port,
                                               iolink_fsm_ds_event_t event)
{
...
static iolink_fsm_ds_event_t ds_reinit (iolink_port_t * port,
                                        iolink_fsm_ds_event_t event)
{
   iolink_ds_port_t * ds = iolink_get_ds_ctx (port);
...
static const iolink_fsm_ds_transition_t ds_trans_s5[] =
{
   ...
   { DS_EVENT_NO_UPLOAD,   DS_STATE_CheckDSValidity,      ds_chk_validity }, /* T20 or T43 */
...
static const iolink_fsm_ds_transition_t ds_trans_s9[] =
{
   ...
   { DS_EVENT_COM_ERR,     DS_STATE_WaitingOnDSActivity,  ds_wait_ds_act   }, /* Not in spec. */ // TODO need to call DS_Fault()?
};
...
static void iolink_ds_event (iolink_port_t * port, iolink_fsm_ds_event_t event)
{
   do
   {
      ...
      if (!next_trans)
      {
         LOG_ERROR (IOLINK_DS_LOG, "%u: next_trans == NULL: DS state %s - DS event %s\n",
      ...
      LOG_DEBUG (IOLINK_DS_LOG, "DS (%u): event: %s, state transition: %s -> %s\n",
                 iolink_get_portnumber (port), iolink_ds_event_literals[event],
                 iolink_ds_state_literals[previous], iolink_ds_state_literals[ds->state]);
...
static void ds_read_cnf_cb (iolink_job_t * job)
{
   ...
   case DS_STATE_CheckIdentity:
      ...
      {
         ds->device_ds.size_max = (job->al_read_cnf.data[0] << 24) +
                                   (job->al_read_cnf.data[1] << 16) +
                                   (job->al_read_cnf.data[2] << 8) +
                                   job->al_read_cnf.data[3];
         if (ds->device_ds.size_max > IOLINK_DS_MAX_SIZE)
         {
            LOG_DEBUG (IOLINK_DS_LOG, "%u: DS: Maximum size is %u, got %lu\n",
                       iolink_get_portnumber (port),
                       IOLINK_DS_MAX_SIZE,
   ...
   case DS_STATE_ReadParameter:
      ...
      {
         if ((ds->current_index == DS_PARAM_INDEX) &&
             (ds->current_subindex == DS_PARAM_SUBINDEX_INDEX_LIST))
         {
            uint8_t i = 0;
         ...
         else
         {
            ...
            if ((ds->master_ds.pos + 2 + 1 + len) < IOLINK_DS_MAX_SIZE)
            {
               ...
               ds->master_ds.pos++;
               memcpy (&ds->master_ds.data[ds->master_ds.pos],
                       job->al_read_cnf.data, len);
               ds->master_ds.pos += len;
            }
            else
            {
               LOG_DEBUG (IOLINK_DS_LOG,
                          "%u: DS: not enough space in data store, max_size = %lu, pos = %u, len = %u\n",
                          iolink_get_portnumber (port),
   ...
   case DS_STATE_CheckUpload:
      ...
      ds->device_ds.cs = (job->al_read_cnf.data[0] << 24) +
                         (job->al_read_cnf.data[1] << 16) +
                         (job->al_read_cnf.data[2] << 8) +
                         job->al_read_cnf.data[3];
                            ...
                            iolink_smi_errortypes_t errortype)
{
   ...
   iolink_post_job_with_type_and_callback (port, job, IOLINK_JOB_AL_READ_CNF, ds_read_cnf_cb);
}
                             ...
                             iolink_smi_errortypes_t errortype)
{
   ...
   iolink_post_job_with_type_and_callback (port, job, IOLINK_JOB_AL_WRITE_CNF, ds_write_cnf_cb);
}
...
iolink_error_t DS_Delete (iolink_port_t * port)
{
   ...
   iolink_post_job_with_type_and_callback (port, job, IOLINK_JOB_DS_DELETE, ds_delete_cb);
...
iolink_error_t DS_Startup (iolink_port_t * port)
{
   ...
   iolink_post_job_with_type_and_callback (port, job, IOLINK_JOB_DS_STARTUP, ds_startup_cb);
...
iolink_error_t DS_Init (iolink_port_t * port,
                        const portconfiglist_t * cfg_list)
{
   ...
   iolink_post_job_with_type_and_callback (port, job, IOLINK_JOB_DS_STARTUP, ds_init_cb);
...
bool DS_Chk_Cfg (iolink_port_t * port, const portconfiglist_t * cfg_list)
{
   ...
   if ((ds->master_ds.vid) &&
       (ds->master_ds.vid == cfg_list->vendorid) &&
                                       ...
                                       arg_block_t * arg_block)
{
   ...
   if (exp_arg_block_id != IOLINK_ARG_BLOCK_ID_VOID_BLOCK ||
       ref_arg_block_id != IOLINK_ARG_BLOCK_ID_DS_DATA)
   {
      smi_error = IOLINK_SMI_ERRORTYPE_ARGBLOCK_NOT_SUPPORTED; // TODO what to use?
   ...
   {
      smi_error = IOLINK_SMI_ERRORTYPE_ARGBLOCK_INCONSISTENT; // TODO what to use?
      iolink_smi_joberror_ind (port, exp_arg_block_id, ref_arg_block_id,
                               smi_error);
      error = IOLINK_ERROR_PARAMETER_CONFLICT;
   }
   else
   {
      ...
      memcpy (ds->master_ds.data, arg_block_ds_data->ds_data,
