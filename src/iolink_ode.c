...
typedef struct iolink_fsm_ode_transition
{
   ...
   iolink_fsm_ode_event_t (*action)(iolink_port_t * port,
                                    iolink_fsm_ode_event_t event);
...
static void ode_AL_Read_cnf (iolink_port_t * port, uint8_t len,
                             const uint8_t * data,
                             iolink_smi_errortypes_t errortype);
static void ode_AL_Write_cnf (iolink_port_t * port,
                              iolink_smi_errortypes_t errortype);
static void SMI_rw_req_cb (iolink_job_t * job);

static iolink_fsm_ode_event_t ode_smi_od_err (iolink_port_t * port,
                                              iolink_fsm_ode_event_t event)
{
   ...
   iolink_arg_block_id_t ref_arg_block_id = smi_req->arg_block->void_block.arg_block_id;

   /* SMI_ParamReadBatch and SMI_ParamWriteBatch are not supported */
   if (ref_arg_block_id == IOLINK_ARG_BLOCK_ID_PORT_INXDEX_LIST ||
   ...
   iolink_smi_joberror_ind (port, smi_req->exp_arg_block_id, ref_arg_block_id, errortype);

   return ODE_EVENT_NONE;
}

static iolink_fsm_ode_event_t ode_od_start (iolink_port_t * port,
                                            iolink_fsm_ode_event_t event)
{
   /* Wait for SMI calls */
   return ODE_EVENT_NONE;
...
static iolink_fsm_ode_event_t ode_od_stop (iolink_port_t * port,
                                           iolink_fsm_ode_event_t event)
{
   return ODE_EVENT_NONE;
}
...
static iolink_fsm_ode_event_t ode_AL_Write_req (iolink_port_t * port)
{
   ...
   AL_Write_req (port, arg_block_od->index, arg_block_od->subindex,
                 arg_block_data_len, arg_block_od->data, ode_AL_Write_cnf);

   return ODE_EVENT_OD_BLOCK;
}

static iolink_fsm_ode_event_t ode_smi_rw (iolink_port_t * port,
                                          iolink_fsm_ode_event_t event)
{
   ...
   if ((exp_arg_block_id == IOLINK_ARG_BLOCK_ID_OD_RD) &&
       (ref_arg_block_id == IOLINK_ARG_BLOCK_ID_OD_RD))
   {
      res_event = ode_AL_Read_req (port);
   }
   else if ((exp_arg_block_id == IOLINK_ARG_BLOCK_ID_VOID_BLOCK) &&
   ...
   {
      iolink_smi_errortypes_t errortype = IOLINK_SMI_ERRORTYPE_ARGBLOCK_NOT_SUPPORTED;
...
static iolink_fsm_ode_event_t ode_smi_done (iolink_port_t * port,
                                            iolink_fsm_ode_event_t event)
{
   iolink_ode_port_t * ode = iolink_get_ode_ctx (port);
   iolink_smi_service_req_t * smi_req = &ode->smi_req;
   ...
   {
      iolink_smi_joberror_ind (port, exp_arg_block_id, ref_arg_block_id, smi_req->result);
   }
   else
   {
      if ((exp_arg_block_id == IOLINK_ARG_BLOCK_ID_OD_RD) &&
          (ref_arg_block_id == IOLINK_ARG_BLOCK_ID_OD_RD))
      {
         iolink_smi_cnf (port, ref_arg_block_id, smi_req->arg_block_len, smi_req->arg_block);
      }
      else if ((exp_arg_block_id == IOLINK_ARG_BLOCK_ID_VOID_BLOCK) &&
               (ref_arg_block_id == IOLINK_ARG_BLOCK_ID_OD_WR))
      {
...
static iolink_fsm_ode_event_t ode_wait (iolink_port_t * port,
                                        iolink_fsm_ode_event_t event)
{
...
static void iolink_ode_event (iolink_port_t * port,
                              iolink_fsm_ode_event_t event)
{
   do
   {
      ...
      if (!next_trans)
      {
         LOG_ERROR (IOLINK_ODE_LOG, "ODE (%u): next_trans == NULL: state %s - event %s\n",
      ...
      LOG_DEBUG (IOLINK_ODE_LOG, "ODE (%u): event: %s, state transition: %s -> %s\n",
                 iolink_get_portnumber (port), iolink_ode_event_literals[event],
                 iolink_ode_state_literals[previous], iolink_ode_state_literals[ode->state]);
...
static iolink_error_t ode_SMI_rw_req (iolink_job_type_t job_type,
                                      iolink_port_t * port,
                             ...
                             iolink_smi_errortypes_t errortype)
{
   ...
   iolink_post_job_with_type_and_callback (port, job, IOLINK_JOB_AL_READ_CNF, ode_read_cnf_cb);
}
                              ...
                              iolink_smi_errortypes_t errortype)
{
   ...
   iolink_post_job_with_type_and_callback (port, job, IOLINK_JOB_AL_WRITE_CNF, ode_write_cnf_cb);
}
...
static void SMI_rw_req_cb (iolink_job_t * job)
{
   ...
   case ODE_STATE_ODblocked:
      ...
      event = (ode->state == ODE_STATE_Inactive) ? ODE_EVENT_SMI_DEV_RW_1 : ODE_EVENT_SMI_DEV_RW_6;
      break;
   default:
      CC_ASSERT (0);
      break;
...
iolink_error_t OD_Stop (iolink_port_t * port)
{
   ...
   iolink_post_job_with_type_and_callback (port, job, IOLINK_JOB_OD_STOP, od_stop_cb);

   return IOLINK_ERROR_NONE;
}

static iolink_error_t ode_SMI_DeviceReadWrite_req (iolink_port_t * port,
                                                   iolink_arg_block_id_t exp_arg_block_id,
                                                   uint16_t arg_block_len,
                                                   arg_block_t * arg_block,
...
iolink_error_t ode_SMI_DeviceRead_req (iolink_port_t * port,
                                       ...
                                       arg_block_t * arg_block)
{
   return ode_SMI_DeviceReadWrite_req(port, exp_arg_block_id, arg_block_len, arg_block,
                                      IOLINK_ARG_BLOCK_ID_OD_RD, IOLINK_JOB_SMI_DEVICE_READ);
}
...
{
   return ode_SMI_DeviceReadWrite_req(port, exp_arg_block_id, arg_block_len, arg_block,
                                      IOLINK_ARG_BLOCK_ID_VOID_BLOCK, IOLINK_JOB_SMI_DEVICE_WRITE);
}
...
iolink_error_t ode_SMI_ParamReadBatch_req (iolink_port_t * port,
                                           ...
                                           arg_block_t * arg_block)
{
   return ode_SMI_rw_req (IOLINK_JOB_SMI_PARAM_READ, port, exp_arg_block_id,
                          arg_block_len, arg_block);
}
...
iolink_error_t ode_SMI_ParamWriteBatch_req (iolink_port_t * port,
                                            ...
                                            arg_block_t * arg_block)
{
   return ode_SMI_rw_req (IOLINK_JOB_SMI_PARAM_WRITE, port, exp_arg_block_id,
                          arg_block_len, arg_block);
}
