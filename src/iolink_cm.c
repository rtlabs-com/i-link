...
typedef struct iolink_fsm_cm_transition
{
   ...
   iolink_fsm_cm_event_t (*action)(iolink_port_t * port,
                                   iolink_fsm_cm_event_t event);
...
static iolink_sm_target_mode_t portmode_to_target_mode (
...
static void portconfiglist_to_parameterlist (const portconfiglist_t * cfg_list,
...
static iolink_fsm_cm_event_t cm_startup (iolink_port_t * port,
                                         iolink_fsm_cm_event_t event)
{
   iolink_cm_port_t * cm = iolink_get_cm_ctx (port);

   if (event == CM_EVENT_IOL_AUTOSTART ||
       !DS_Chk_Cfg (port, &cm->cfg_list)) /* CM_EVENT_IOL_MANUAL */
   {
      DS_Delete (port);
   }
...
static iolink_fsm_cm_event_t cm_dido (iolink_port_t * port,
                                      iolink_fsm_cm_event_t event)
{
...
static iolink_fsm_cm_event_t cm_check_mode (iolink_port_t * port,
                                            iolink_fsm_cm_event_t event)
{
   ...
   if (event == CM_EVENT_CFG_CHANGE)
   {
      ...
      if (res_event == CM_EVENT_UNKNOWN)
      {
         ...
         iolink_smi_joberror_ind (port, exp_arg_block_id, ref_arg_block_id,
...
static inline void iolink_cm_do_smi_port_event (iolink_port_t * port,
                                                iolink_eventcode_t eventcode)
{
   ...
   iolink_smi_cnf (port, IOLINK_ARG_BLOCK_ID_VOID_BLOCK,
                   sizeof(arg_block_portevent_t), (arg_block_t *)&port_event);
}

static iolink_fsm_cm_event_t cm_comlost (iolink_port_t * port,
                                         iolink_fsm_cm_event_t event)
{
   iolink_port_info_t * port_info = iolink_get_port_info (port);
...
static iolink_fsm_cm_event_t cm_comlost_ignore (iolink_port_t * port,
                                                iolink_fsm_cm_event_t event)
{
...
static iolink_fsm_cm_event_t cm_cfg_change_ignore (iolink_port_t * port,
                                                   iolink_fsm_cm_event_t event)
{
...
static iolink_fsm_cm_event_t cm_ds_change (iolink_port_t * port,
                                           iolink_fsm_cm_event_t event)
{
...
static void init_port_info(iolink_port_t * port,
                           iolink_port_status_info_t port_status_info,
                           iolink_port_quality_info_t port_quality_info)
{
   iolink_port_info_t * port_info = iolink_get_port_info (port);
   const iolink_smp_parameterlist_t * real_paramlist = iolink_get_paramlist (port);
...
static iolink_fsm_cm_event_t cm_start_ds (iolink_port_t * port,
                                          iolink_fsm_cm_event_t event)
{
   init_port_info(port, IOLINK_PORT_STATUS_INFO_PREOP,
...
static iolink_fsm_cm_event_t cm_ds_ready (iolink_port_t * port,
                                          iolink_fsm_cm_event_t event)
{
   SM_Operate (port);
...
static iolink_fsm_cm_event_t cm_ds_ready_ignore (iolink_port_t * port,
                                                 iolink_fsm_cm_event_t event)
{
...
static iolink_fsm_cm_event_t cm_sm_operate (iolink_port_t * port,
                                            iolink_fsm_cm_event_t event)
{
   // TODO Update parameter elements of PortStatusList
   // Port QualityINfo = x
   init_port_info(port, IOLINK_PORT_STATUS_INFO_OP,
         IOLINK_PORT_QUALITY_INFO_VALID);
...
static iolink_fsm_cm_event_t cm_port_fault (iolink_port_t * port,
                                            iolink_fsm_cm_event_t event)
{
   ...
   init_port_info(port, IOLINK_PORT_STATUS_INFO_PORT_DIAG,
         IOLINK_PORT_QUALITY_INFO_INVALID);
...
static iolink_fsm_cm_event_t cm_deactivate (iolink_port_t * port,
                                            iolink_fsm_cm_event_t event)
{
   iolink_cm_port_t * cm = iolink_get_cm_ctx (port);
   portconfiglist_t * cfg_list = &cm->cfg_list;
   ...
   init_port_info(port, IOLINK_PORT_STATUS_INFO_DEACTIVATED,
         IOLINK_PORT_QUALITY_INFO_INVALID);
...
/* since we iterate through the list of events put the most likely in the top of the list. */
...
static const iolink_fsm_cm_transition_t cm_trans_s2[] =
{
   ...
   { CM_EVENT_CFG_CHANGE,    CM_STATE_DS_ParamManager,  cm_cfg_change_ignore }, /* Not in spec */
   { CM_EVENT_SM_COMLOST,    CM_STATE_DS_ParamManager,  cm_comlost_ignore    }, /* Not in spec */
};
...
static void iolink_cm_event (iolink_port_t * port, iolink_fsm_cm_event_t event)
{
   do
   {
      ...
      if (!next_trans)
      {
         LOG_ERROR (IOLINK_CM_LOG, "CM (%u): next_trans == NULL: state %s - event %s\n",
      ...
      LOG_DEBUG (IOLINK_CM_LOG, "CM (%u): event: %s, state transition: %s -> %s\n",
                 iolink_get_portnumber (port), iolink_cm_event_literals[event],
                 iolink_cm_state_literals[previous], iolink_cm_state_literals[cm->state]);
   ...
   case IOLINK_SM_PORTMODE_INACTIVE:
      {
         ...
         if (port_info->port_status_info != IOLINK_PORT_STATUS_INFO_DEACTIVATED)
         {
            ...
            /* SMI_PortEvent_ind */
            iolink_cm_do_smi_port_event (port,
...
static bool check_arg_block(iolink_job_t * job,
      iolink_arg_block_id_t cmp_ref_arg_block_id, uint16_t arg_block_len,
      iolink_arg_block_id_t cmp_exp_arg_block_id, iolink_smi_errortypes_t errortype)
{
   iolink_port_t * port = job->port;
...
static void cm_smi_masterident_cb (iolink_job_t * job)
{
   ...
   if (check_arg_block(job, IOLINK_ARG_BLOCK_ID_VOID_BLOCK, sizeof(arg_block_void_t),
         IOLINK_ARG_BLOCK_ID_MASTERIDENT, IOLINK_SMI_ERRORTYPE_NONE))
   {
      ...
      iolink_smi_cnf (port, ref_arg_block_id, arg_block_len,
...
static void cm_smi_portconfiguration_cb (iolink_job_t * job)
{
   ...
   if (check_arg_block(job, IOLINK_ARG_BLOCK_ID_PORT_CFG_LIST,
...
static void cm_smi_readbackportconfiguration_cb (iolink_job_t * job)
{
   ...
   if (check_arg_block(job, IOLINK_ARG_BLOCK_ID_VOID_BLOCK,
         sizeof(arg_block_void_t), IOLINK_ARG_BLOCK_ID_PORT_CFG_LIST,
         IOLINK_SMI_ERRORTYPE_ARGBLOCK_INCONSISTENT))
   {
      iolink_cm_port_t * cm = iolink_get_cm_ctx (port);
...
static void cm_smi_portstatus_cb (iolink_job_t * job)
{
   ...
   if (check_arg_block(job, IOLINK_ARG_BLOCK_ID_VOID_BLOCK, sizeof(arg_block_void_t),
         IOLINK_ARG_BLOCK_ID_PORT_STATUS_LIST, IOLINK_SMI_ERRORTYPE_NONE))
   {
      ...
      if (port_status.port_status_info == IOLINK_PORT_STATUS_INFO_NO_DEV ||
      ...
      // TODO for each diag entry .. number_of_diags++ and append to diag_entries
...
void DS_Ready (iolink_port_t * port)
{
   ...
   iolink_post_job_with_type_and_callback (port, job, IOLINK_JOB_DS_READY, cm_ds_ready_cb);
}
...
void DS_Change (iolink_port_t * port)
{
   ...
   iolink_post_job_with_type_and_callback (port, job, IOLINK_JOB_DS_CHANGE, cm_ds_change_cb);
}
...
void DS_Fault (iolink_port_t * port, iolink_ds_fault_t fault)
{
   ...
   iolink_post_job_with_type_and_callback (port, job, IOLINK_JOB_DS_FAULT, cm_ds_fault_cb);
}

static iolink_error_t init_and_post_job(iolink_port_t * port, iolink_job_type_t type,
      void (*callback)(struct iolink_job * job), iolink_arg_block_id_t exp_arg_block_id,
...
{
   return init_and_post_job(port, IOLINK_JOB_SMI_MASTERIDENT,
         cm_smi_masterident_cb, exp_arg_block_id, arg_block_len, arg_block);
}
...
iolink_error_t cm_SMI_PortConfiguration_req (iolink_port_t * port,
                                       ...
                                       arg_block_t * arg_block)
{
   return init_and_post_job(port, IOLINK_JOB_SMI_PORTCONFIGURATION,
         cm_smi_portconfiguration_cb, exp_arg_block_id, arg_block_len, arg_block);
}
...
iolink_error_t cm_SMI_ReadbackPortConfiguration_req (iolink_port_t * port,
                                        ...
                                        arg_block_t * arg_block)
{
   return init_and_post_job(port, IOLINK_JOB_SMI_READBACKPORTCONFIGURATION,
         cm_smi_readbackportconfiguration_cb, exp_arg_block_id, arg_block_len, arg_block);
}
...
iolink_error_t cm_SMI_PortStatus_req (iolink_port_t * port,
                                      ...
                                      arg_block_t * arg_block)
{
   return init_and_post_job(port, IOLINK_JOB_SMI_PORTSTATUS,
         cm_smi_portstatus_cb, exp_arg_block_id, arg_block_len, arg_block);
}
