...
static inline void pde_verify_smi_err (iolink_arg_block_id_t exp_smi_ref_arg_id,
...
TEST_F (PDETest, pde_PDIn_inactive)
{
   ...
   iolink_arg_block_id_t exp_exp_arg_block_id = IOLINK_ARG_BLOCK_ID_FS_MASTER_ACCESS;
   ...
   EXPECT_EQ (IOLINK_ERROR_PARAMETER_CONFLICT,
              SMI_PDIn_req (portnumber, exp_exp_arg_block_id,
                            sizeof(arg_block_void_t),
                            (arg_block_t *)&arg_block_void));
   EXPECT_EQ (PD_STATE_Inactive, pde_get_state (port));

   pde_verify_smi_err (ref_arg_block_id, exp_exp_arg_block_id,
                       IOLINK_SMI_ERRORTYPE_ARGBLOCK_INCONSISTENT,
                       0, exp_joberror_cnt);
}
...
TEST_F (PDETest, pde_PDOut_inactive)
{
   ...
   EXPECT_EQ (IOLINK_ERROR_STATE_INVALID,
              SMI_PDOut_req (portnumber, exp_exp_arg_block_id,
...
static inline void pde_verify_smi_in (iolink_arg_block_id_t exp_smi_ref_arg_id,
                           iolink_arg_block_id_t exp_smi_arg_id,
                           iolink_port_qualifier_info_t exp_port_qualifier_info,
                           uint8_t exp_len, uint8_t * exp_data,
                           uint8_t exp_data_len)
{
   ...
   if (exp_smi_arg_id == IOLINK_ARG_BLOCK_ID_PD_IN)
   {
      ...
      EXPECT_EQ (exp_port_qualifier_info,
                 arg_block_pdin->h.port_qualifier_info);
...
static inline void pde_verify_smi_out (iolink_arg_block_id_t exp_smi_ref_arg_id,
                                       iolink_arg_block_id_t exp_smi_arg_id,
                                       uint8_t exp_len)
{
   EXPECT_EQ (exp_smi_ref_arg_id, mock_iolink_smi_ref_arg_block_id);
...
{
   return &arg_block->
            data[2 + iolink_arg_block_pdinout_pdin_data_len (arg_block)];
}
                           ...
                           uint8_t exp_pdout_data_len)
{
   ...
   if (exp_smi_arg_id == IOLINK_ARG_BLOCK_ID_PD_IN_OUT)
   {
      ...
      EXPECT_TRUE (ArraysMatchN (exp_pdin_data, pdin_data,
                   exp_pdin_data_len));
   }
   else
   {
...
static void pde_smi_pdout (iolink_port_t * port, uint8_t oe, uint8_t data_len,
                           uint8_t * data)
{
   iolink_pde_port_t * pde = iolink_get_pde_ctx (port);
   ...
   EXPECT_EQ (IOLINK_ERROR_NONE,
              SMI_PDOut_req (iolink_get_portnumber (port), exp_exp_arg_block_id,
                             arg_block_len, (arg_block_t *)&arg_block_pdout));

   EXPECT_EQ (exp_smi_cnf_cnt, mock_iolink_smi_cnf_cnt);
   pde_verify_smi_out (ref_arg_block_id, exp_exp_arg_block_id,
                       sizeof(arg_block_void_t));
...
TEST_F (PDETest, pde_PDIn)
{
   ...
   uint8_t data[16] = { 1, 3, 7, 2, 5, 4, 9, 6,
                        8, 11, 19, 17, 12, 7, 12, 2 };
   ...
   EXPECT_EQ (exp_smi_cnf_cnt, mock_iolink_smi_cnf_cnt);
   pde_verify_smi_in (ref_arg_block_id, exp_exp_arg_block_id,
   ...
   memcpy (mock_iolink_dl_pdin_data, data, exp_data_len);
   EXPECT_EQ (IOLINK_ERROR_NONE,
              AL_Control_ind (port, IOLINK_CONTROLCODE_VALID));
...
TEST_F (PDETest, pde_PDInOut)
{
   ...
   uint8_t pdin_data[16] = { 1, 3, 7, 2, 5, 4, 9, 6,
                             8, 11, 19, 17, 12, 7, 12, 2 };
   ...
   arg_block_void.arg_block_id = ref_arg_block_id;
   EXPECT_EQ (IOLINK_ERROR_NONE,
              SMI_PDInOut_req (portnumber, exp_exp_arg_block_id,
                               sizeof(arg_block_void_t),
                               (arg_block_t *)&arg_block_void));
   ...
   EXPECT_EQ (exp_smi_cnf_cnt, mock_iolink_smi_cnf_cnt);
   pde_verify_smi_in_out (ref_arg_block_id, exp_exp_arg_block_id,
                          IOLINK_PORT_QUALIFIER_INFO_PQ_VALID,
                          exp_output_enable,
                          sizeof(arg_block_pdinout_head_t) +
                          1 + exp_pdin_data_len + 1,
                          pdin_data, exp_pdin_data_len,
                          NULL, 0);
   ...
   pde_verify_smi_in_out (ref_arg_block_id, exp_exp_arg_block_id,
                          ...
                          exp_output_enable,
                          sizeof(arg_block_pdinout_head_t) +
                          1 + exp_pdin_data_len + 1 + exp_pdout_data_len,
...
TEST_F (PDETest, pde_PDIn_bad_argblock)
{
   ...
   pde_verify_smi_err (bad_arg_block_id,  exp_exp_arg_block_id,
