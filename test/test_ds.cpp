...
static inline iolink_error_t par_serv_to_ds (iolink_port_t * port, uint16_t len,
                                             const uint8_t * data, uint16_t vid,
                                             uint32_t did)
{
   ...
   return SMI_ParServToDS_req (iolink_get_portnumber (port),
                               IOLINK_ARG_BLOCK_ID_VOID_BLOCK,
                               arg_block_ds_data_len,
                               (arg_block_t *)arg_block_ds_data);
}

static inline void ds_verify_id (iolink_port_t * port, uint16_t vid,
                                 uint32_t did)
{
   iolink_ds_port_t * ds = iolink_get_ds_ctx (port);
...
static inline void ds_check_fault (iolink_port_t * port,
                                   iolink_ds_fault_t expected_fault)
{
   if (expected_fault == IOLINK_DS_FAULT_DOWN ||
       expected_fault == IOLINK_DS_FAULT_UP)
   {
...
static inline void ds_state_0_to_4 (iolink_port_t * port, uint16_t len,
                                    const uint8_t * data, uint16_t vid,
                                    uint32_t did,
...
static inline void ds_state_0_to_5 (iolink_port_t * port, uint16_t len,
                                    const uint8_t * data, uint16_t vid,
                                    uint32_t did, uint8_t * ds_size,
                                    iolink_validation_check_t validation)
{
...
static inline void ds_state_0_to_6 (iolink_port_t * port, uint16_t len,
                                    const uint8_t * data, uint16_t vid,
                                    uint32_t did, uint8_t * ds_size,
                                    iolink_validation_check_t validation,
                                    uint8_t * state_property)
{
   ...
   mock_iolink_al_read_cnf_cb (port, 1, state_property,
                               IOLINK_SMI_ERRORTYPE_NONE);
   mock_iolink_job.callback (&mock_iolink_job);
}

static inline void ds_state_0_to_10 (iolink_port_t * port, uint16_t len,
                                     const uint8_t * data, uint16_t vid,
                                     uint32_t did, uint8_t * ds_size,
                                     iolink_validation_check_t validation,
                                     uint8_t * state_property)
{
   ...
   ds_state_0_to_6 (port, len, data, vid, did, ds_size, validation,
                    state_property);

   EXPECT_EQ (DS_STATE_CheckUpload, ds_get_state (port));
   // Read checksum
   mock_iolink_al_read_cnf_cb (port, sizeof(checksum),
                               (uint8_t *)checksum,
                               IOLINK_SMI_ERRORTYPE_NONE);
   mock_iolink_job.callback (&mock_iolink_job);
...
TEST_F (DSTest, Off_DS_Delete)
{
   ...
   EXPECT_EQ (IOLINK_ERROR_NONE,
              par_serv_to_ds (port, sizeof(data_store_data),
                              data_store_data, vid, did));
...
TEST_F (DSTest, DS_download)
{
   ...
   uint8_t data_store_data[] = { 0x01, 0x23, 0x00, 0x02, 0x12, 0x34, 0x01, 0x24,
                                 0x01, 0x01, 0x55, 0x01, 0x24, 0x02, 0x01, 0xaa,
                               };
   ...
   ds_state_0_to_10 (port, sizeof(data_store_data), data_store_data, vid, did,
                     data_storage_size, IOLINK_VALIDATION_CHECK_V11_RESTORE,
                     state_property);
...
TEST_F (DSTest, DS_upload_upload_req)
{
   ...
   ds_state_0_to_5 (port, 0, NULL, 0, 0, data_storage_size,
   ...
   EXPECT_EQ (mock_iolink_al_read_req_cnt, 2);
   // Read device State_Property
   mock_iolink_al_read_cnf_cb (port, sizeof(state_property),
                               state_property,
                               IOLINK_SMI_ERRORTYPE_NONE);
   mock_iolink_job.callback (&mock_iolink_job);
   EXPECT_EQ (DS_STATE_CheckUpload, ds_get_state (port));
   ...
   mock_iolink_al_read_cnf_cb (port, sizeof(index_list),
                               (uint8_t *)index_list,
                               IOLINK_SMI_ERRORTYPE_NONE);
   mock_iolink_job.callback (&mock_iolink_job);
   ...
   for (i = 0; i < ARRAY_SIZE (index_list) - 1; i++)
   {
      ...
      mock_iolink_al_read_cnf_cb (port, sizeof(param_data),
                                  param_data,
                                  IOLINK_SMI_ERRORTYPE_NONE);
      mock_iolink_job.callback (&mock_iolink_job);
   }
...
TEST_F (DSTest, DS_Fault_ID_read_error)
{
   ...
   mock_iolink_al_read_cnf_cb (port, 0, NULL,
...
TEST_F (DSTest, DS_Fault_Size)
{
   ...
   ds_state_0_to_5 (port, sizeof(data_store_data), data_store_data, vid, did,
                    data_storage_size, IOLINK_VALIDATION_CHECK_V11_RESTORE);
...
TEST_F (DSTest, DS_Fault_UL_Read_Param_Dev_Err)
{
   ...
   EXPECT_EQ (mock_iolink_al_read_req_cnt, 2);
   // Read device State_Property
   mock_iolink_al_read_cnf_cb (port, sizeof(state_property), state_property,
                               IOLINK_SMI_ERRORTYPE_NONE);
   mock_iolink_job.callback (&mock_iolink_job);
   EXPECT_EQ (DS_STATE_CheckUpload, ds_get_state (port));
...
TEST_F (DSTest, DS_Fault_DL_Write_Param_Dev_Err)
{
   ...
   ds_state_0_to_10 (port, sizeof(data_store_data), data_store_data, vid, did,
                     data_storage_size, IOLINK_VALIDATION_CHECK_V11_BAK_RESTORE,
                     state_property);
...
TEST_F (DSTest, DS_Chk_Cfg_vid)
{
   ...
   EXPECT_EQ (IOLINK_ERROR_NONE, par_serv_to_ds (port, sizeof(data_store_data),
                                                 data_store_data, vid, did));
