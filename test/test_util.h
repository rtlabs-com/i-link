...
template<typename T>
::testing::AssertionResult ArraysMatchN(const T (*expected),
                                       const T (*actual),
                                       const size_t size)
{
   for (size_t i(0); i < size; ++i)
   {
      if (expected[i] != actual[i])
      {
         return ::testing::AssertionFailure()
            << "actual[" << i
            << "] (" << FormatByte (static_cast<int>(actual[i]))
            << ") != expected[" << i
            << "] (" << FormatByte (static_cast<int>(expected[i]))
            << ")";
      }
   }
...
inline ::testing::AssertionResult EventMatch (al_event_t * e1,
                                              diag_entry_t * diag,
                                              uint8_t event_cnt)
{
   int i;
   ...
   for (i = 0; i < event_cnt; i++)
   {
      ...
      if (e1->eventcode != e2->eventcode)
         return ::testing::AssertionFailure()
            << i << ": eventcode: (" << e2->eventcode
            << ") != expected (" << e1->eventcode
            << ")";
      if (e1->instance != e2->instance)
         return ::testing::AssertionFailure()
            << i << ": instance: (" << +e2->instance
            << ") != expected (" << +e1->instance
            << ")";
      if (e1->mode != e2->mode)
         return ::testing::AssertionFailure()
            << i << ": mode: (" << +e2->mode
                                          ...
                                          const portconfiglist_t * actual)
{
   if (expected->portmode != actual->portmode)
      return ::testing::AssertionFailure()
         << "portmode (" << actual->portmode
         << ") != expected (" << expected->portmode
         << ")";
   ...
   if (expected->iq_behavior != actual->iq_behavior)
      return ::testing::AssertionFailure()
         << "iq_behavior (" << actual->iq_behavior
         << ") != expected (" << expected->iq_behavior
         << ")";
   ...
   if (expected->in_buffer_len != actual->in_buffer_len)
      return ::testing::AssertionFailure()
         << "in_buffer_len (" << actual->in_buffer_len
         << ") != expected (" << expected->in_buffer_len
         << ")";
